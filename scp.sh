...
#} WINCH
#spawn scp -r killer@192.168.1.82:/home/killer/Work/CPG_New/linux/*.csv .
#expect "password:"
#send   killer\r
#interact
...
for server in $AUTO_SSH_CONFIG; do
	...
	if [ $i -eq $no ] ; then
        ...
        #/home/killer/Work/CPG_New/linux
        FILE_PATH=`echo $server | awk -F\| '{ print $7 }'`
		FILE='/tmp/.login.sh'
        ...
        echo '} WINCH' >> $FILE
        # spawn scp -r killer@192.168.1.82:/home/killer/Work/CPG_New/linux/*.csv .
        echo "spawn scp -r "$NAME@$IP":"$FILE_PATH"/*.csv ." >> $FILE
        if [ "$PASS" != "" ]; then
